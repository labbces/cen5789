# Genômica e Bioinformática - CEN5789
Aulas práticas da disciplina ["CEN5789 - Genômica e Bioinformática"](https://uspdigital.usp.br/janus/componente/disciplinasOferecidasInicial.jsf?action=3&sgldis=CEN5789)

__Criador e Instrutor do material de Bioinformática e tradução do material de UNIX__
Diego M. Riaño-Pachón

__Criadores do material de LINUX na versão em Inglês__  
Simon Prochnik  
Sofia Robb 

# Unix
* [Unix](unix.md/#unix)
   * [Unix 1](unix.md/#unix-1)
      * [Panorama de Unix](unix.md/#panorama-de-unix)
         * [O que é a linha de comandos?](unix.md/#o-que-é-a-linha-de-comandos)
      * [O básico](unix.md/#o-básico)
         * [Formas de acessar o terminal do Linux](unix.md/#formas-de-acessar-o-terminal-do-linux)
         * [OK. Estou no terminal. E agora?](unix.md/#ok-estou-no-terminal-e-agora)
         * [Prompt da linha de comandos](unix.md/#prompt-da-linha-de-comandos)
         * [Executando comandos](unix.md/#executando-comandos)
         * [Edição da linha de comandos](unix.md/#edição-da-linha-de-comandos)
         * [Curingas](unix.md/#curingas)
         * [Lar doce lar](unix.md/#lar-doce-lar)
         * [Conhecendo o sistema de arquivos](unix.md/#conhecendo-o-sistema-de-arquivos)
         * [Comandos essenciais de Unix](unix.md/#comandos-essenciais-de-unix)
         * [Obtendo informação dos comandos](unix.md/#obtendo-informação-dos-comandos)
         * [Encontrando quais comandos estão disponíveis no seu computador](unix.md/#encontrando-quais-comandos-estão-disponíveis-no-seu-computador)
         * [Argumentos e switches na linha de comando](unix.md/#argumentos-e-switches-na-linha-de-comando)
         * [Espaços e caracteres estranhos](unix.md/#espaços-e-caracteres-estranhos)
         * [Comandos úteis](unix.md/#comandos-úteis)
         * [Manipulando pastas](unix.md/#manipulando-pastas)
         * [Rede](unix.md/#rede)
         * [E/S padrão e redirecionamento](unix.md/#es-padrão-e-redirecionamento)
         * [Um exemplo simples](unix.md/#um-exemplo-simples)
         * [Meta-caracteres de redirecionamento](unix.md/#meta-caracteres-de-redirecionamento)
         * [Filtros, Nomes de arquivos, e entrada padrão](unix.md/#filtros-nomes-de-arquivos-e-entrada-padrão)
         * [E/S Padrão e Tubos](unix.md/#es-padrão-e-tubos)
         * [Mais Exemplos de Encadeamento de Comandos (Pipes)](unix.md/#mais-exemplos-de-encadeamento-de-comandos-pipes)
      * [Mais Unix](unix.md/#mais-unix)
      * [<a href="problemsets/Unix_01_problemset.md">Link para o conjunto de problemas 1 de Unix</a>](problemsets/Unix_01_problemset.md)
   * [Unix 2](unix.md/#unix-2)
      * [Editores de texto](unix.md/#editores-de-texto)
      * [Introdução ao nano](unix.md/#introdução-ao-nano)
         * [Usando nano](unix.md/#usando-nano)
      * [Introdução ao vi](unix.md/#introdução-ao-vi)
         * [Começando com o vi](unix.md/#começando-com-o-vi)
         * [Criando, Escrevendo, e salvando o percorrer de um arquivo](unix.md/#criando-escrevendo-e-salvando-o-percorrer-de-um-arquivo)
         * [Atividades comuns e comandos vi](unix.md/#atividades-comuns-e-comandos-vi)
         * [Outras dicas úteis](unix.md/#outras-dicas-úteis)
         * [Caneca do vi](unix.md/#caneca-do-vi)
   * [Git para iniciantes](unix.md/#git-para-iniciantes)
      * [A visão geral.](unix.md/#a-visão-geral)
         * [Colaboração](unix.md/#colaboração)
         * [Armazenando versões](unix.md/#armazenando-versões)
         * [Restaurando Versões Anteriores](unix.md/#restaurando-versões-anteriores)
         * [Backup](unix.md/#backup)
         * [Os detalhes](unix.md/#os-detalhes)
      * [O Básico](unix.md/#o-básico-1)
      * [Criando um novo repositório](unix.md/#criando-um-novo-repositório)
         * [Mantendo o monitoramento de diferenças entre repositórios locais e remotos](unix.md/#mantendo-o-monitoramento-de-diferenças-entre-repositórios-locais-e-remotos)
         * [Deletando e movendo arquivos](unix.md/#deletando-e-movendo-arquivos)
         * [Conseguir uma cópia do arquivo em seu remoto](unix.md/#conseguir-uma-cópia-do-arquivo-em-seu-remoto)
         * [Dicas](unix.md/#dicas)
      * [Clonando um Repositório](unix.md/#clonando-um-repositório)
         * [Trazendo mudanças do repositório remoto para o local](unix.md/#trazendo-mudanças-do-repositório-remoto-para-o-local)
         * [Links para tópicos <em>levemente</em> menos básicos](unix.md/#links-para-tópicos-levemente-menos-básicos)
      * [<a href="problemsets/Unix_02_problemset.md">Link para o conjunto de problemas Unix 2</a>](problemsets/Unix_02_problemset.md)
* [Bioinformática](bioinfo.md/#bioinformática)
   * [Bioinfo 1 - Operações básicas em Bioinformática](bioinfo.md/#bioinfo-1---operações-básicas-em-bioinformática)
      * [Ferramentas do Unix úteis na bioinformática.](bioinfo.md/#ferramentas-do-unix-úteis-na-bioinformática)
         * [Algumas operações básicas com arquivos.](bioinfo.md/#algumas-operações-básicas-com-arquivos)
         * [Formatos de sequências](bioinfo.md/#formatos-de-sequências)
            * [Fasta](bioinfo.md/#fasta)
            * [GenBank](bioinfo.md/#genbank)
         * [Algumas operações básicas com sequências no formato Fasta](bioinfo.md/#algumas-operações-básicas-com-sequências-no-formato-fasta)
   * [Bioinfo 2 - Análise de Sequências](bioinfo.md/#bioinfo-2---análise-de-sequências)
      * [Dotplots - Matrices de pontos](bioinfo.md/#dotplots---matrices-de-pontos)
         * [Sequências repetidas](bioinfo.md/#sequências-repetidas)
         * [Domínios proteicos conservados](bioinfo.md/#domínios-proteicos-conservados)
         * [Éxons e Íntrons](bioinfo.md/#éxons-e-íntrons)
         * [Dotplots com EMBOSS e outros applicativos](bioinfo.md/#dotplots-com-emboss-e-outros-applicativos)
      * [Alinhamentos exatos de pares de sequências](bioinfo.md/#alinhamentos-exatos-de-pares-de-sequências)
         * [cDNA vs Genoma](bioinfo.md/#cdna-vs-genoma)
         * [Proteínas](bioinfo.md/#proteínas)
            * [Matrices de substituição](bioinfo.md/#matrices-de-substituição)
            * [NACs](bioinfo.md/#nacs)
      * [Alinhamentos heurísticos de pares de sequências - Buscas em bancos de dados](bioinfo.md/#alinhamentos-heurísticos-de-pares-de-sequências---buscas-em-bancos-de-dados)
         * [BLAST na interface web](bioinfo.md/#blast-na-interface-web)
         * [BLAST na linha de comandos](bioinfo.md/#blast-na-linha-de-comandos)
   * [Bioinfo 3 - Análise de dados de sequenciamento em larga escala](bioinfo.md/#bioinfo-3---análise-de-dados-de-sequenciamento-em-larga-escala)
      * [Avaliando a qualidade de sequencias Illumina](bioinfo.md/#avaliando-a-qualidade-de-sequencias-illumina)
         * [qc1.fq.gz](bioinfo.md/#qc1fqgz)
         * [qc2.fq.gz](bioinfo.md/#qc2fqgz)
         * [qc3.fq.gz](bioinfo.md/#qc3fqgz)
      * [Análise de espectros de k-mers](bioinfo.md/#análise-de-espectros-de-k-mers)
         * [Contando k-mers](bioinfo.md/#contando-k-mers)
         * [GenomeScope2](bioinfo.md/#genomescope2)
         * [SmudgePlot](bioinfo.md/#smudgeplot)
   * [Bioinfo 4 - Montagem <em>de novo</em> de genomas](bioinfo.md/#bioinfo-4---montagem-de-novo-de-genomas)
      * [Obtendo os dados e calculando métricas do genoma](bioinfo.md/#obtendo-os-dados-e-calculando-métricas-do-genoma)
      * [A montar!](bioinfo.md/#a-montar)
         * [Hifiasm](bioinfo.md/#hifiasm)
         * [Flye](bioinfo.md/#flye)
      * [Examinando as montagens.](bioinfo.md/#examinando-as-montagens)
         * [Métricas de continuidade](bioinfo.md/#métricas-de-continuidade)
         * [Métricas de completude](bioinfo.md/#métricas-de-completude)
            * [Espaço gênico](bioinfo.md/#espaço-gênico)
         * [Telômeros](bioinfo.md/#telômeros)
      * [Comparando as montagens](bioinfo.md/#comparando-as-montagens)
   * [Bioinfo 5 -  Genome annotation](bioinfo.md/#bioinfo-5----genome-annotation)
      * [Realize a avaliação de contaminações.](bioinfo.md/#realize-a-avaliação-de-contaminações)
      * [Mascare repetições](bioinfo.md/#mascare-repetições)
      * [Obter evidência extrínseca - Proteínas de espécies próximas.](bioinfo.md/#obter-evidência-extrínseca---proteínas-de-espécies-próximas)
      * [Anotando o Genoma - GALBA](bioinfo.md/#anotando-o-genoma---galba)
   * [Bioinfo 6 - Gene Ortólogos](bioinfo.md/#bioinfo-6---gene-ortólogos)
   * [Bioinfo 7 - Transcriptômica](bioinfo.md/#bioinfo-7---transcriptômica)
      * [Descarregando os dados de repositórios públicos](bioinfo.md/#descarregando-os-dados-de-repositórios-públicos)
      * [Pre-processando os dados de RNASeq](bioinfo.md/#pre-processando-os-dados-de-rnaseq)
      * [Estimando os Níveis de Expressão de Transcritos e Genes](bioinfo.md/#estimando-os-níveis-de-expressão-de-transcritos-e-genes)
      * [Vamos nos preparar para a detecção de GENES diferencialmente expressos.](bioinfo.md/#vamos-nos-preparar-para-a-detecção-de-genes-diferencialmente-expressos)
      * [Carregando os dados em R](bioinfo.md/#carregando-os-dados-em-r)
      * [DESeq2](bioinfo.md/#deseq2)
         * [Modelo estatístico no DESeq2](bioinfo.md/#modelo-estatístico-no-deseq2)
         * [Identificacão de genes diferencialmente expressos com DESseq2](bioinfo.md/#identificacão-de-genes-diferencialmente-expressos-com-desseq2)
         * [Diagramas de Venn dos genes differencialmente expressos](bioinfo.md/#diagramas-de-venn-dos-genes-differencialmente-expressos)
         * [Identificando funções sobre-representadas nos grupos de genes diferencialmente expressos.](bioinfo.md/#identificando-funções-sobre-representadas-nos-grupos-de-genes-diferencialmente-expressos)
         * [Identificando grupos de genes com perfis de expressão semelhantes.](bioinfo.md/#identificando-grupos-de-genes-com-perfis-de-expressão-semelhantes)
